...
# stt_interfaces 패키지의 Stt, SttMatched 서비스 임포트
from abc_interfaces.srv import Stt, SttMatched
...
class VlmLogicNode(Node):
    def __init__(self):
        ...
        # ── STT 매칭 성공 서비스 서버 ──────────────────────────────
        # /stt_matched 서비스 서버 생성 (SttMatched.srv 타입)
        # STT에서 매칭 성공한 결과를 직접 DB 조회용으로 수신
        ...
        self.get_logger().info("VLM Logic 노드 가동 완료. STT 서비스(/stt_results, /stt_matched) 대기 중...")
    ...
    def stt_service_callback(self, request: Stt.Request, response: Stt.Response):
        ...
        self.process_targets_with_db(target_list, source="VLM")
    ...
    def stt_matched_service_callback(self, request: SttMatched.Request, response: SttMatched.Response):
        ...
        STT가 이미 상품 매칭을 성공했으므로, 바로 DB 재고 조회를 수행하고
        재고 충분/부족에 따라 처리한다. VLM은 처리하지 않음.
        """
        ...
        self.get_logger().info(
            f"\n[STT 매칭 성공 - VLM 처리 안 함] class_name={list(request.class_name)}, "
            f"iteration={list(request.iteration)} 수신됨. 바로 DB 조회 시작..."
        )

        # STT 매칭 성공한 결과를 target_list 형식으로 변환
        target_list = [
            {"class_name": cn, "iteration": it}
            for cn, it in zip(request.class_name, request.iteration)
        ]

        # DB 조회 및 재고 판단 (기존 로직 재사용)
        self.process_targets_with_db(target_list, source="STT")

        response.success = True
        response.message = "STT 매칭 성공: DB 조회 시작"
        return response

    # ── 요구사항 2~4: DB 조회 + 재고 판단 + 서비스 전송 ────────────
    def process_targets_with_db(self, target_list: list, source: str = "VLM"):
        """
        target_list: [{"class_name": str, "iteration": int}, ...]
        source: "VLM" (Gemini 분석/VLM 처리함) 또는 "STT" (STT 직접 매칭/VLM 처리 안 함)
        ...
        """
        source_label = "VLM으로 분석 처리" if source == "VLM" else "STT 직접 매칭 (VLM 처리 안 함)"
        self.get_logger().info(f"▶ [{source_label}] DB 재고 조회 시작...")
        ...
        if insufficient_class_names:
            self.get_logger().warn(
                f"▶ [{source_label}] 재고 부족 상품 발생 → /vlm_to_tts 서비스 전송\n"
        ...
        self.get_logger().info(
            f"▶ [{source_label}] 모든 상품 재고 충분 → /vlm_request 서비스 전송\n"
